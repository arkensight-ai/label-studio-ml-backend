...
from aule.groundingdino.util.misc import nested_tensor_from_tensor_list

from aule.groundingdino.util.box_ops import box_xyxy_to_cxcywh

def collate_grounding(batch: list[dict]) -> dict:
...
def reverse_normalization(normalized_img_tensor) -> torch.Tensor:
    return F.normalize(
        normalized_img_tensor, mean=[-m/s for m,s in zip(IMG_MEAN, IMG_STD)],
        std=[1/s for s in IMG_STD]
    )
    
def make_transforms(dataset_mode='train', augmentation_config=None, subtask_config=None):
    """Build Augmentations.
    ...
    if augmentation_config is None:
        augmentation_config = {}
    input_mean = list(augmentation_config.get("input_mean", IMG_MEAN))
    input_std = list(augmentation_config.get("input_std", IMG_STD))
    scales = list(augmentation_config.get("scales", [480, 512, 544, 576, 608, 640, 672, 704, 736, 768, 800]))
    random_resize_max_size = augmentation_config.get("random_resize_max_size", 1333)
    test_random_size = augmentation_config.get("test_random_resize", 800)
    train_random_sizes = list(augmentation_config.get("train_random_resize", [400, 500, 600]))
    train_random_crop_min = augmentation_config.get("train_random_crop_min", 384)
    train_random_crop_max = augmentation_config.get("train_random_crop_max", 600)
    flip_prob = min(1.0, augmentation_config.get("horizontal_flip_prob", 0.5))
    fixed_padding = augmentation_config.get("fixed_padding", False)

    # ViT requires square input. D2 LSJ style data transforms
    fixed_random_crop = augmentation_config.get("fixed_random_crop", None)  
    normalize = Compose([
        ToTensor(),
    ...
    elif dataset_mode in ('val', 'eval', 'infer', 'test', 'predict', 'validation'):
        if fixed_padding:
            transforms = Compose([
...
def reverse_padding(image: torch.Tensor, original_image_size: int, size: int = 800, max_size: int = 1333) -> torch.Tensor:
    size = get_size(original_image_size, size, max_size)
