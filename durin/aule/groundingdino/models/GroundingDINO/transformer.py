...
from aule.groundingdino.util.misc import inverse_sigmoid
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # Begin Encoder
        #########################################################
        assert not src_flatten.isnan().any(), "Src going into encoder has nans"
        ...
        assert not memory.isnan().any(), "Memory from encoder has nans"
        assert not memory_text.isnan().any(), "Text memory from encoder has nans"
        #########################################################
        # End Encoder
        # - memory: bs, \sum{hw}, c
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            #         import ipdb; ipdb.set_trace()
            assert not output.isnan().any(), f"Output has nans in layer: {layer_id}"
            assert (
                not memory_text.isnan().any()
            ), f"Memory text has nans is layer: {layer_id}"
            if self.fusion_layers:
                ...
                assert (
                    not output.isnan().any() and not memory_text.isnan().any()
                ), f"Fusion layer {layer_id} gives nans"
